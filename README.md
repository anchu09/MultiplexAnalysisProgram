...
Python pipeline for spatial tumor analysis combining two complementary imaging modalities. Developed in collaboration with the Confocal Microscopy Unit at CNIO (Spanish National Cancer Research Centre).
...
**Module 2 — H&E Invasion-Front Detection:** deep-learning module that localizes tumor-invasion fronts directly on routine Hematoxylin-and-Eosin slides, providing precise tumor–stroma boundaries without extensive manual annotation. Achieves **macro F1 = 0.82** and **AUC > 0.90** on HNSCC tissue.

---

## What the data looks like

Each region of interest (ROI) from an HNSCC biopsy contains up to 22 fluorescence channels. The image below overlays Pan-Cytokeratin (CK, magenta — tumor epithelium), DAPI (blue — nuclei), CD3 (green — T cells), CD4 (red), and FOXP3 (yellow — Tregs).
...
4. Tissue mask generation  preprocessing/segmentation.py create_channel_masks()
      │
      ▼  labeled masks — each cell gets a unique integer ID
...
Nuclei are segmented by Cellpose and each cell is assigned a unique integer ID. This labeled mask is the spatial anchor for all downstream analysis.

![Labeled cell mask](docs/images/fig05_labeled_mask.png)

*Left (A): DAPI channel with Cellpose outlines. Right (B): pseudo-colored instance mask — each cell has a unique color; background is black.*

### Tumor mask generation

CK intensity is thresholded at the ROI mean (k = 0) to delineate the tumor epithelium. Small debris is removed and holes are filled, giving a binary mask where CK⁺ pixels mark tumor area.
...
Each query cell is assigned to the Voronoi polygon of its nearest reference cell, giving an exact nearest-neighbor distance between the two populations.
...
![Tumor characterization](docs/images/fig25_tumor_characterization.png)
...
Treg (CD3⁺ CD4⁺ FOXP3⁺) centroids with their signed-distance vectors to the CK boundary. Red = excluded from tumor, green = infiltrating.
...
*Tregs (colored by distance to nearest reference cell) vs CK⁺ NGFR⁺ (A) and CK⁺ NGFR⁻ (B). Dark red = close; orange/yellow = far.*

---

## H&E Invasion-Front Detection

Tumor-invasion fronts — the epithelial margins where active invasion into the stroma occurs — are the most biologically significant region in a biopsy, yet their manual delineation is time-consuming and observer-dependent. This module automates their detection on FFPE H&E whole-slide images.
...
*Semi-automatic annotation workflow on an HNSCC H&E section. **(A)** Original H&E tile with manually traced invasion-front epithelium (yellow/orange overlay) — polygons drawn in QuPath 0.6.3 and validated by a pathologist. **(B)** Inverted non-front mask: stromal tissue (purple) and background glass (bright areas) remain unassigned at this stage. **(C)** Final three-class ground-truth map: red = tumor-invasion front (class 1), yellow = background glass (class 0), gray/purple = stroma (class 2). Every pixel is labeled, giving 100% ground-truth coverage per slide.*

### Stain decomposition

Each RGB tile is decomposed into hematoxylin (nuclear detail) and eosin (cytoplasmic/stromal detail) optical-density channels, giving the model explicit stain signals rather than raw color values.

![Stain decomposition](docs/images/fig15_hne_stain_decomposition.png)

*H&E color deconvolution (Ruifrok & Johnston, 2001). The RGB tile is separated into two optical-density channels: **hematoxylin** (blue — nuclear staining, dense tumor nests appear darker) and **eosin** (pink — cytoplasmic and stromal proteins). These two channels replace the raw RGB input to the U-Net, providing explicit stain signals that generalize better across scanners and staining batches.*

### Architecture

U-Net with Squeeze-and-Excitation (SE) attention and dilated convolutions (`hne/models/unet.py`):

- **Input:** 2-channel float tensor — normalized H + E optical-density maps.
...
| Learning rate | 1×10⁻⁴ (fixed) | Configured via `LEARNING_RATE` in `hne/config.py` |
| Loss | Cross-entropy (ignore\_index=−1) | Ignores unannotated pixels; class imbalance handled by oversampling |
...
| Checkpoint | Best val loss | Saved to path passed to `train_model`; no save if `checkpoint_path=None` |
...
*Normalized confusion matrix (recall per row). Background is nearly perfectly separated (recall 0.95) — bright glass has a characteristic photometric signature that the model identifies reliably. The hardest distinction is between invasion front and stroma: 11% of true invasion-front pixels are misclassified as stroma, and 20% of stroma pixels are misclassified as invasion front, reflecting the gradual morphological transition at the tumor–stroma interface.*
...
*Representative test patch (2 048 × 2 048 px, HNSCC biopsy). **Left:** original H&E tile. **Center:** model prediction after morphological post-processing. **Right:** expert ground truth. Color key: orange = background glass, red = tumor invasion front, green = stroma. The model correctly delineates the invasion front along the epithelial margin and cleanly excludes the background; minor discrepancies appear at stromal boundaries where the transition is morphologically ambiguous.*
...
│
├── tests/
│   ├── test_intensity.py         # Cell intensity extraction + binary flags
│   ├── test_spatial.py           # Mask area, centroids, distance functions
│   ├── test_overlays.py          # IF visualization smoke tests
│   ├── test_pipeline_e2e.py      # Full IF pipeline end-to-end integration
...
│                                                # Note: outputs cleared before commit (patient data).
│                                                # Run against your own data to reproduce figures.

│
├── groovy/
│   ├── cellpose_seg.groovy           # QuPath script: run Cellpose nuclei segmentation
│   └── Export_DAPI_to_Mask.groovy    # QuPath script: export labeled DAPI masks
│
...
| `CHARACTERIZATION_COMBINATIONS` | Tumor characterization subpopulations |
...
from multiplex_pipeline.domain import CHANNELS_OF_INTEREST, MARKER_LABELS, INTENSITY_THRESHOLDS
import pandas as pd
...
from multiplex_pipeline.domain import CHARACTERIZATION_COMBINATIONS
...
counts = plot_combination_counts(
    ...
    plot_title="Tumor Characterization — CK⁺ Subpopulations",
)
```
...
from multiplex_pipeline.config import PIXEL_SIZE
from multiplex_pipeline.domain import (
    SUBPOPULATION_A_POSITIVE, SUBPOPULATIONS, CONDITION_COLUMN_MAP, SHADING_COLORS,
)
...
pytest tests/test_hne_models.py tests/test_hne_preprocessing.py tests/test_hne_metrics.py -v   # H&E (requires ml group)
ruff check .                # linting
