...
def compute_confusion_matrix(
    ...
    ----------
    y_true, y_pred:
        Flat integer arrays of true and predicted labels.
...
def compute_iou_dice(
    ...
    ----------
    y_true, y_pred:
        Flat integer arrays (IGNORE_INDEX pixels are excluded).
...
def compute_roc_auc(
    ...
    ----------
    y_true:
        Flat integer array of ground-truth labels.
    y_score:
        Float array of shape (N_pixels, n_classes) with class probabilities.
    max_pixels:
        Maximum pixels to use; if N > max_pixels a random subset is drawn.
    seed:
        Random seed for sub-sampling reproducibility.
    ...
    if n > max_pixels:
        random.seed(seed)
        idx = random.sample(range(n), k=max_pixels)
        y_true_s = y_true[idx]
...
def compute_per_patch_iou(
    ...
    Parameters
    ----------
    y_true_patches, y_pred_patches:
        Lists of flat 1-D arrays, one per patch.

    Returns
    -------
    np.ndarray of shape (n_patches, n_classes) with IoU values.
        Zero is used when denominator is zero (class absent in both arrays).
    """
    ious = []
