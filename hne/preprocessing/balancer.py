...
2. Purging – stroma-only patches are removed until stroma pixel count
   no longer exceeds tumor pixel count.
"""
...
def oversample_tumor_patches(
    ...
    All patches are written to dst_dir (RGB, hematoxylin, eosin, and mask).
    Patches with more than min_tumor_fraction tumor pixels are also duplicated
    oversample_factor - 1 times using random geometric augmentations.

    Args:
        src_dir: Directory containing patch_NNNN.png and mask_NNNN.png files.
        dst_dir: Destination directory (created if it does not exist).
        min_tumor_fraction: Minimum fraction of tumor pixels (class 1) to oversample.
        oversample_factor: Total copies (original + augmented) per tumor patch.
        seed: Random seed used for augmentation sampling.
    """
    ...
    with ThreadPoolExecutor(max_workers=MAX_CPU_WORKERS) as pool:
        ...
        for fut in tqdm(as_completed(futures), total=len(futures), desc="Copying originals"):
            fut.result()  # raise any exception from the worker thread
    ...
    with ThreadPoolExecutor(max_workers=MAX_CPU_WORKERS) as pool:
        ...
        for fut in tqdm(as_completed(futures), total=len(futures), desc="Augmenting tumor patches"):
            fut.result()  # raise any exception from the worker thread
...
def balance_dataset(
    ...
    Args:
        src_dir: Directory produced by `oversample_tumor_patches`.
        dst_dir: Output directory for the balanced patch set.
        seed: Random seed used to shuffle the final kept-patch order.
    """
    src_dir, dst_dir = Path(src_dir), Path(dst_dir)
    dst_dir.mkdir(parents=True, exist_ok=True)
    random.seed(seed)
    ...
    with ThreadPoolExecutor(max_workers=MAX_CPU_WORKERS) as pool:
        ...
        for fut in tqdm(as_completed(futures), total=len(futures), desc="Copying balanced patches"):
            fut.result()  # raise any exception from the worker thread
...
def _tumor_fraction(mask_path: Path) -> float:
    ...
    if mask is None:
        return 0.0
    return float((mask == 1).sum()) / float(mask.size)
...
def _process_augmented(patch_path: Path, mask_path: Path, new_num: int, dst_dir: Path) -> None:
    ...
    _write_quad(aug_rgb, h_u8, e_u8, aug_mask, f"{new_num:04d}", dst_dir)
...
def _write_quad(
    ...
    cv2.imwrite(str(dst_dir / f"{HEMATOXYLIN_PREFIX}_{num}.png"), hema)
    cv2.imwrite(str(dst_dir / f"{EOSIN_PREFIX}_{num}.png"), eosin)
...
def _copy_quad(ident: str, new_idx: int, src_dir: Path, dst_dir: Path) -> None:
    ...
    for prefix in ("patch", HEMATOXYLIN_PREFIX, EOSIN_PREFIX, "mask"):
