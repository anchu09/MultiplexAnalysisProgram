...
and extracts non-overlapping tiles, rasterising polygon annotations into
...
class RobustPatchExtractor:
    ...
    ----------
    img_path:
        Directory containing the OME-TIFF file.
    ome_filename:
        Name of the OME-TIFF file inside img_path.
    ann_path:
        Directory containing QuPath GeoJSON annotation files (searched
        recursively).
    output_dir:
        Where to write patch_NNNN.png and mask_NNNN.png files.
    patch_size:
        Tile edge length in pixels.
    stride:
        Step between tile origins. Equal to patch_size means no overlap.
    tissue_threshold:
        Minimum fraction of foreground (dark) pixels required to keep a tile.
    class_map:
        Mapping from annotation class name to integer label.
    hole_threshold:
        Mean RGB value above which unannotated pixels are classified as
        background (0); below → stroma (2).
    """
    ...
    def __init__(
        ...
        _required_keys = {"ignore-fondo", "Stroma"}
        _missing = _required_keys - self.class_map.keys()
        if _missing:
            raise ValueError(
                f"class_map is missing required keys: {_missing}. "
                "Expected keys: 'ignore-fondo' (background glass) and 'Stroma'."
            )
    ...
    def is_tissue(self, patch: np.ndarray) -> bool:
        ...
        return (thr == 0).mean() > self.tissue_threshold

    def rasterize_mask(self, x0: int, y0: int) -> np.ndarray:
        """Rasterise annotation polygons into a tile-sized label mask.

        Unannotated pixels are initialised to 255 (sentinel for holes).
        Interior rings (polygon holes) are also reset to 255.
        """
    ...
    def _process_task(
        ...
        if np.any(mask != 255):
            ...
            if hole_mask.any():
                ...
                mask[hole_mask & (brightness <= self.hole_threshold)] = self.class_map["Stroma"]
