...
from multiplex_pipeline.preprocessing.segmentation import apply_dapi_mask
...
def simple_dapi_mask():
    """5×5 labeled mask with two regions (labels 1 and 2)."""
...
def roi_data(simple_dapi_mask, simple_image):
    roi = "ROI1"
    dapi_masks = {dapi_key(roi.lower()): simple_dapi_mask}
...
class TestLabelDapi:
    def test_binary_mask_gets_labeled(self):
        binary = np.array([[0, 1, 0], [1, 1, 0], [0, 0, 0]], dtype=int)
        result = label_dapi(binary)
        assert result.max() >= 1

    def test_already_labeled_mask_returned_unchanged(self, simple_dapi_mask):
...
class TestProcessRoi:
    ...
    def test_raises_on_shape_mismatch(self, roi_data):
        roi_data["dapi_masks"][dapi_key("roi1")] = np.zeros((3, 3), dtype=int)
...
class TestApplyDapiMask:
    def test_require_dapi_false_returns_mask_unchanged(self):
        initial = np.ones((5, 5), dtype=bool)
        result = apply_dapi_mask(initial, "1", {}, require_dapi=False, name="ROI1.ome.tiff")
        assert result is initial

    def test_require_dapi_true_missing_mask_returns_none(self, caplog):
        initial = np.ones((5, 5), dtype=bool)
        with caplog.at_level(logging.WARNING):
            result = apply_dapi_mask(initial, "1", {}, require_dapi=True, name="ROI1.ome.tiff")
        assert result is None
        assert dapi_key("roi1") in caplog.text

    def test_require_dapi_true_applies_intersection(self):
        initial = np.ones((3, 3), dtype=bool)
        dapi = np.zeros((3, 3), dtype=np.uint8)
        dapi[0, 0] = 1  # only top-left pixel is a cell
        result = apply_dapi_mask(
            initial, "1", {dapi_key("roi1"): dapi}, require_dapi=True, name="ROI1"
        )
