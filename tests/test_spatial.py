...
@pytest.fixture()
def labeled_dapi():
    """3×3 labeled DAPI mask with two single-pixel regions."""
...
class TestComputeMaskAreaSummary:
    def test_returns_dataframe_with_expected_columns(self, square_masks):
        ...
        result = compute_mask_area_summary({"roi1": ck}, {"roi1": ngfr}, pixel_area_um2=1.0)
    ...
    def test_total_area_is_full_mask(self, square_masks):
        ck, ngfr = square_masks
        result = compute_mask_area_summary({"roi1": ck}, {"roi1": ngfr}, pixel_area_um2=2.0)
        assert result.iloc[0][TOTAL_AREA_ROI_UM2] == pytest.approx(9 * 2.0)

    def test_pixel_area_scales_output(self, square_masks):
        ck, ngfr = square_masks
        r1 = compute_mask_area_summary({"roi1": ck}, {"roi1": ngfr}, pixel_area_um2=1.0)
        r2 = compute_mask_area_summary({"roi1": ck}, {"roi1": ngfr}, pixel_area_um2=0.5)
    ...
    def test_missing_ngfr_roi_skipped(self, square_masks, caplog):
        ck, _ = square_masks
        result = compute_mask_area_summary({"roi1": ck}, {}, pixel_area_um2=1.0)
        assert result.empty

    def test_shape_mismatch_skipped(self, square_masks, caplog):
        ck, _ = square_masks
        ngfr_wrong = np.ones((5, 5), dtype=np.uint8)
        result = compute_mask_area_summary({"roi1": ck}, {"roi1": ngfr_wrong}, pixel_area_um2=1.0)
        assert result.empty
...
class TestGetCentroids:
    def test_returns_dataframe(self, labeled_dapi):
        result = get_centroids(labeled_dapi)
        assert isinstance(result, pd.DataFrame)

    def test_expected_columns(self, labeled_dapi):
        result = get_centroids(labeled_dapi)
        assert set(result.columns) == {DAPI_ID, CENTROID_ROW, CENTROID_COL}

    def test_one_row_per_region(self, labeled_dapi):
        result = get_centroids(labeled_dapi)
        assert len(result) == 2
