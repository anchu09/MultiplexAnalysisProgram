...
def parse_distance_matrix_filename(
    ...
    This function is specific to NGFR-stratified distance matrices produced by
    compute_and_plot_subpop_distances_for_all_rois. Files for other markers or
    naming conventions will return None.
...
def load_distance_matrices_for_plot(
    ...
    """Load and organize distance matrix CSVs for combined boxplot generation.
    ...
    for sub in base_path.iterdir():
        ...
        for csv_file in sub.glob("*.csv"):
            ...
            key = SIGN_PLUS if sign == "+" else SIGN_MINUS
...
def filter_cells_by_combination(
    df: pd.DataFrame,
    combination_func: Callable[[pd.DataFrame], pd.Series],
) -> pd.DataFrame:
