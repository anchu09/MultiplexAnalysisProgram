...
Re-exports both submodules' public API so callers need only one import.
"""
...
from multiplex_pipeline.config import DISTANCES_POPULATIONS_DIR, PIXEL_SIZE, RANDOM_STATE
...
def _render_roi_figure(
    roi: str,
    img_data: np.ndarray,
    cell_mask: np.ndarray,
    df_roi: pd.DataFrame,
    selected_cells: list,
    mask_dicts_combined: dict[str, dict[str, np.ndarray]],
    markers_to_plot: list[tuple[str, int | None]],
    parsed_conditions: dict[str, int],
    condition_column_map: dict[str, str],
    conditions: list[str],
    brightness_factor: float,
) -> None:
    """Render and display the two-row QC figure for a single ROI.

    Top row: composite + per-marker channels/masks + mean.
    ...
    rand_colors = rng.random((len(cell_labels) + 1, 3))
    rand_colors[0] = [0, 0, 0]
    cmap_cells = ListedColormap(rand_colors)

    top_images: list[np.ndarray | None] = [None] * n_cols
    bottom_images: list[np.ndarray | None] = [None] * n_cols
    ...
    for i, (marker_name, ch) in enumerate(markers_to_plot, start=2):
        ...
        if _is_mask_marker(marker_name):
            ...
            if ch is not None:
                ...
                channel_norm = (
                    (channel_data - cmin) / (cmax - cmin)
                    if cmax > cmin
                    else np.zeros_like(channel_data)
                )
    ...
    if top_images[0] is None:
        raise RuntimeError(f"Composite image was not built for ROI '{roi}'.")
    ...
    for j, (marker_name, _) in enumerate(markers_to_plot, start=2):
        ...
        if background is None:
            raise RuntimeError(f"Channel image {j - 1} was not built for ROI '{roi}'.")
        ...
        if col_bin is not None:
            expected_val = parsed_conditions.get(col_bin, 1)
            selected_for_marker = df_roi[df_roi[col_bin] == expected_val][DAPI_ID].tolist()
        else:
            selected_for_marker = []
    ...
    if selected_cells:
        cmap_selected = ListedColormap(["black"] + list(rng.random((len(selected_cells), 3))))
...
def plot_conditional_cells_channels(
    ...
    ck_masks_dict: dict[str, np.ndarray],
    ngfr_masks_dict: dict[str, np.ndarray],
    condition_column_map: dict[str, str],
    brightness_factor: float = 1.0,
) -> None:
    """Plot cells and channels for the specified conditions in each ROI.

    Renders a two-row figure per ROI via `_render_roi_figure`:
    ...
    markers_to_plot = [(cond[:-1], _get_channel_for_marker(cond[:-1])) for cond in conditions]

    for roi in rois:
        logger.info("Processing %s...", roi)
        roi_lower = roi.lower()
        roi_dapi_key = dapi_key(roi_lower)
...
def plot_roi_split_markers(
    ...
    precomputed_merged: if provided, skips the regionprops/centroid merge step
        (useful when the caller already built it, e.g. compute_and_save).

    Returns:
        Tuple (fig_ck, fig_ngfr), either of which may be None if skipped.
    """
    dapi = dapi_masks.get(dapi_key(roi))
    if dapi is None:
...
def compute_and_save(
    ...
    out_dir = str(Path(path_save) / subpop_name)
    Path(out_dir).mkdir(parents=True, exist_ok=True)

    # Build centroids once — passed to plot_roi_split_markers to avoid recomputing.
    dapi = dapi_masks.get(dapi_key(roi))
    if dapi is None:
        logger.warning("compute_and_save: no DAPI mask for '%s' — skipping.", roi)
        return
    ck = ck_masks.get(roi, np.zeros_like(dapi, dtype=np.uint8))
    ngfr = ngfr_masks.get(roi, np.zeros_like(dapi, dtype=np.uint8))
    merged = _build_centroid_dataframe(dapi, df_bin, roi)
    ...
    if fig_ck:
        fig_ck.savefig(str(Path(out_dir) / f"roi_{roi}_ck.svg"))
        show_figure(fig_ck)
    if fig_ngfr:
        fig_ngfr.savefig(str(Path(out_dir) / f"roi_{roi}_ngfr.svg"))
        show_figure(fig_ngfr)
    ...
    csv_path = str(Path(out_dir) / f"roi_{roi}_distance_table.csv")
...
def compute_and_plot_subpop_distances_for_all_rois(
    rois: list[str],
    subpop_conditions_A: list[str],
    subpop_conditions_B: list[str],
    df_binary: pd.DataFrame,
    dapi_masks_dict: dict[str, np.ndarray],
    condition_column_map: dict[str, str],
    ...
    plot_type: str = "line",
    subpop_b_label: str | None = None,
) -> dict[str, pd.DataFrame]:
    """Compute pairwise A×B distances and render overlay plots for every ROI.

    Distances are in µm (pixel distances × pixel_size). Optionally writes
    a pivot-table CSV per ROI. Use plot_type='voronoi' for Voronoi tessellation
    or 'line' for nearest-neighbor arrows.

    subpop_b_label: display label for subpopulation B in titles and filenames.

    Returns:
        Dict mapping ROI name → pairwise distances DataFrame.
    """
    subpop_a_name = " & ".join(subpop_conditions_A)
    subpop_b_name = (
        subpop_b_label if subpop_b_label is not None else " & ".join(subpop_conditions_B)
    )
    ...
    all_distances: dict[str, pd.DataFrame] = {}

    for roi in rois:
        logger.info("Processing ROI %s", roi)
        dapi_mask_key = dapi_key(roi.lower())
        if dapi_mask_key not in dapi_masks_dict:
            logger.warning("Missing DAPI mask for %s. Skipping.", dapi_mask_key)
            continue

        dapi_mask = dapi_masks_dict[dapi_mask_key]
        ...
        subpop_a = select_subpopulation(df_roi, parsedA)
        subpop_b = select_subpopulation(df_roi, parsedB)

        try:
            dist_df = compute_subpop_distances(subpop_a, subpop_b)
        ...
        if save_matrix_as_csv:
            csv_filename = f"distance_matrix_{roi}_{subpop_a_name}_vs_{subpop_b_name}.csv"
            Path(path_save).mkdir(parents=True, exist_ok=True)
            csv_filepath = str(Path(path_save) / csv_filename)
            distance_matrix.to_csv(csv_filepath, index=True)
            logger.info("Matrix saved: %s", csv_filepath)

        if len(subpop_a) > 0 and len(subpop_b) > 0:
            plot_filename = str(
                Path(path_save) / f"plot_{roi}_{subpop_a_name}_vs_{subpop_b_name}.svg"
            )
            plot_subpopulations_and_distances(
                roi=roi,
                dapi_mask=dapi_mask,
                subpop_a_df=subpop_a,
                subpop_b_df=subpop_b,
                dist_df=dist_df,
                subpop_a_name=subpop_a_name,
                subpop_b_name=subpop_b_name,
                options=SubpopPlotOptions(
